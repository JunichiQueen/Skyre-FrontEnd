                ...
                <Tab.Container id="left-tabs-example" defaultActiveKey="first">
                    <Row>
                        ...
                        <Col sm={9}>
                            <Tab.Content>
                                {this.state.nav.map((item) => (
                                    <Tab.Pane eventKey={`${item.citizenId}`}><p>{item.forenames} <SearchSaveModal firstname={item.forenames} lastname={item.surname} address={item.homeAddress}
                                    placeOfBirth={item.placeOfBirth} dateOfBirth={item.dateOfBirth} sex={item.sex} citizenId={item.citizenId} /></p></Tab.Pane>
                                ))}
                            </Tab.Content>
                        </Col>
                    </Row>
